// Package currency provides currency definitions, formatting, and parsing
// for monetary values in invoices.
package currency
...
// Currencies contains predefined currency definitions.
...
// Get returns the currency for a given code.
func Get(code string) (Currency, bool) {
	c, ok := Currencies[strings.ToUpper(code)]
	return c, ok
}

// Formatter formats monetary amounts according to currency rules.
type Formatter struct {
	currency Currency
}

// NewFormatter creates a new currency formatter for the given currency code.
...
// Format formats a decimal amount according to the currency's rules.
...
// Parse parses a formatted currency string into a decimal amount.
...
// FormatSimple formats an amount with the given currency code using defaults.
