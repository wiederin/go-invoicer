// Package render provides PDF rendering capabilities for invoices,
// supporting both template-based and simple structured rendering.
package render
...
// Options configures PDF rendering settings.
type Options struct {
	PageSize     string
	Orientation  string
	MarginTop    float64
...
// DefaultOptions returns sensible default PDF options.
...
// NewEngine creates a new rendering engine with the given template manager.
...
// RenderInvoice renders an invoice to PDF using the specified template.
...
// SimpleRenderer renders invoices directly to PDF without templates.
type SimpleRenderer struct {
	Options Options
}

// NewSimpleRenderer creates a new simple renderer with default options.
...
// RenderInvoice renders an invoice to PDF bytes.
...
// RenderToWriter renders an invoice and writes the PDF to the given writer.
